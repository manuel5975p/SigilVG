...
// The per-vertex input data consists of 7 attributes all having 4 floating-point components:
//
// 0 - pos
// 1 - tex
// 2 - jac
// 3 - bnd
// 4 - col
// 5 - grad0 (gradient endpoints in em-space)
// 6 - grad1 (gradient texture row, type, radius, spread)
...
// col = vertex color (red, green, blue, alpha). Negative alpha = gradient mode.
// grad0 = gradient direction params: linear(x1,y1,x2,y2), radial(cx,cy,fx,fy)
// grad1 = (gradTexRow, gradType, radius, spread)
...
struct VertexInput {
    ...
    @location(5) grad0: vec4<f32>,                   // attrib[5]
    @location(6) grad1: vec4<f32>,                   // attrib[6]
};
